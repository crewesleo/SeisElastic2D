    ...
    if(any_elastic) then
      ...
      if (save_ASCII_kernels) then ! ascii format
        ...
        if (ios /= 0) stop 'Error writing kernel file to disk'
          ...
          if (.not. ANISO .and. (trim(M_PAR)=='isodv' .OR. trim(M_PAR)=='isodm')) then
            ...
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_Qkappa_kernel.bin'
            open(unit = 210,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_Qmu_kernel.bin'
            ...
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_vs.bin'
            open(unit = 288,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_rhop.bin'
            open(unit = 289,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_kappa.bin'
            open(unit = 290,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_mu.bin'
            open(unit = 291,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_rho.bin'
            open(unit = 292,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'
          endif

          if (.not. ANISO .and. (trim(M_PAR)=='isodl')) then
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_dl_lambda_kernel.bin'
            open(unit = 272, file = 'OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_dl_mu_kernel.bin'
            open(unit = 273, file ='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_dl_rho_kernel.bin'
            open(unit = 274, file ='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_dl_lambda.bin'
            open(unit = 293,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_dl_mu.bin'
            open(unit = 294,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_dl_rho.bin'
            open(unit = 295,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'
          endif
          if (.not. ANISO .and. (trim(M_PAR)=='isodip')) then
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_dip_ip_kernel.bin'
            open(unit = 275, file = 'OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_dip_is_kernel.bin'
            open(unit = 276, file ='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_dip_rho_kernel.bin'
            open(unit = 277, file ='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_dip_ip.bin'
            open(unit = 296,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_dip_is.bin'
            open(unit = 297,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_dip_rho.bin'
            open(unit = 298,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'
          endif

          if (.not. ANISO .and. (trim(M_PAR)=='isovipi')) then
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_vipi_vp_kernel.bin'
            open(unit = 278, file = 'OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_vipi_vs_kernel.bin'
            open(unit = 279, file ='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_vipi_ip_kernel.bin'
            open(unit = 280, file ='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_vipi_vp.bin'
            open(unit = 299,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_vipi_vs.bin'
            open(unit = 300,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_vipi_ip.bin'
            open(unit = 301,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'
          endif
          if (.not. ANISO .and. (trim(M_PAR)=='isovipii')) then
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_vipii_vp_kernel.bin'
            open(unit = 281, file = 'OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_vipii_vs_kernel.bin'
            open(unit = 282, file ='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_vipii_is_kernel.bin'
            open(unit = 283, file ='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_vipii_vp.bin'
            open(unit = 302,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_vipii_vs.bin'
            open(unit = 303,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_vipii_is.bin'
            open(unit = 304,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'
          endif
          if (.not. ANISO .and. (trim(M_PAR)=='isosigma')) then
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_sigma_vp_kernel.bin'
            open(unit = 284, file = 'OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_sigma_sigma_kernel.bin'
            open(unit = 285, file ='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_sigma_rho_kernel.bin'
            open(unit = 286, file ='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_sigma_vp.bin'
            open(unit = 305,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_sigma_sigma.bin'
            open(unit = 306,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_sigma_rho.bin'
            open(unit = 307,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'
          endif

          if (ANISO .and. (trim(M_PAR)=='ttiec') ) then
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ec_rho_kernel.bin'
            open(unit = 258,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ec_c11_kernel.bin'
            open(unit = 259, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ec_c13_kernel.bin'
            open(unit = 260,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ec_c15_kernel.bin'
            open(unit = 261,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ec_c33_kernel.bin'
            open(unit = 262,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ec_c35_kernel.bin'
            open(unit = 263,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ec_c55_kernel.bin'
            open(unit = 264,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ec_rho.bin'
            open(unit = 265,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ec_c11.bin'
            open(unit = 266,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ec_c13.bin'
            open(unit = 267,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ec_c15.bin'
            open(unit = 268,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ec_c33.bin'
            open(unit = 269,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ec_c35.bin'
            open(unit = 270,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ec_c55.bin'
            open(unit = 271,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

          endif
          if (ANISO .and. (trim(M_PAR)=='ttiecu') ) then

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ecu_c11_kernel.bin'
            open(unit = 308, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ecu_c13_kernel.bin'
            open(unit = 309, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ecu_c33_kernel.bin'
            open(unit = 310, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ecu_c55_kernel.bin'
            open(unit = 311, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ecu_theta_kernel.bin'
            open(unit = 312, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_ecu_rho_kernel.bin'
            open(unit = 313, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ecu_c11.bin'
            open(unit = 314, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ecu_c13.bin'
            open(unit = 315, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ecu_c33.bin'
            open(unit = 316, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ecu_c55.bin'
            open(unit = 317, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ecu_theta.bin'
            open(unit = 318, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_ecu_rho.bin'
            open(unit = 319, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

          endif
          if (ANISO .and. (trim(M_PAR)=='ttithom') ) then
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_thom_alpha_kernel.bin'
            open(unit = 320, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_thom_beta_kernel.bin'
            open(unit = 321, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_thom_epsilon_kernel.bin'
            open(unit = 322, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_thom_delta_kernel.bin'
            open(unit = 323, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_thom_theta_kernel.bin'
            open(unit = 324, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_tti_thom_rhop_kernel.bin'
            open(unit = 325, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_thom_alpha.bin'
            open(unit = 326, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_thom_beta.bin'
            open(unit = 327, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_thom_epsilon.bin'
            open(unit = 328, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_thom_delta.bin'
            open(unit = 329, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_thom_theta.bin'
            open(unit = 330, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_tti_thom_rhop.bin'
          ...
          if (ANISO .and. (trim(M_PAR)=='htiec' .OR. trim(M_PAR)=='vtiec') ) then
            ...
            open(unit = 216,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_ec_c11_kernel.bin'
            open(unit = 217, file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted', iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_ec_c13_kernel.bin'
            open(unit = 218,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_ec_c33_kernel.bin'
            open(unit = 219,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_ec_c55_kernel.bin'
            open(unit = 220,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_ec_c11.bin'
            open(unit = 226,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_ec_c13.bin'
            open(unit = 227,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_ec_c33.bin'
            open(unit = 228,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_ec_c55.bin'
            open(unit = 229,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_ec_rho.bin'
            open(unit = 230,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

          endif
          if (ANISO .and. (trim(M_PAR)=='htithom' .OR. trim(M_PAR)=='vtithom') ) then
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_thom_rhop_kernel.bin'
            open(unit = 221,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_thom_alpha_kernel.bin'
            open(unit = 222,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_thom_beta_kernel.bin'
            open(unit = 223,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'
 
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_thom_epsilon_kernel.bin'
            open(unit = 224,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_thom_delta_kernel.bin'
            open(unit = 225,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_thom_alpha.bin'
            open(unit = 231,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_thom_beta.bin'
            open(unit = 232,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_thom_epsilon.bin'
            open(unit = 233,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_thom_delta.bin'
            open(unit = 234,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_thom_rhop.bin'
            open(unit = 235,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

          endif
          if (ANISO .and. (trim(M_PAR)=='htithom2' .OR. trim(M_PAR)=='vtithom2') ) then
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_thom2_rhop_kernel.bin'
            open(unit = 236,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_thom2_alpha_kernel.bin'
            open(unit = 237,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_thom2_beta_kernel.bin'
            open(unit = 238,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_thom2_epsilon_kernel.bin'
            open(unit = 239,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_thom2_delta_kernel.bin'
            open(unit = 240,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_thom2_eta_kernel.bin'
            open(unit = 241,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_thom2_alpha.bin'
            open(unit = 242,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_thom2_beta.bin'
            open(unit = 243,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_thom2_epsilon.bin'
            open(unit = 244,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_thom2_delta.bin'
            open(unit = 245,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_thom2_rhop.bin'
            open(unit = 246,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_thom2_eta.bin'
            open(unit = 247,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'
          endif
          if (ANISO .and. (trim(M_PAR)=='htivel' .OR. trim(M_PAR)=='vtivel') ) then
            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_vel_rhop_kernel.bin'
            open(unit = 248,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_vel_alpha_kernel.bin'
            open(unit = 249,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_vel_beta_kernel.bin'
            open(unit = 250,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_vel_alphah_kernel.bin'
            open(unit = 251,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_hti_vel_alphan_kernel.bin'
            open(unit = 252,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_vel_alpha.bin'
            open(unit = 253,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_vel_beta.bin'
            open(unit = 254,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_vel_alphah.bin'
            open(unit = 255,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_vel_alphan.bin'
            open(unit = 256,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'

            write(outputname,'(a,i6.6,a)') 'proc',myrank,'_pdh_hti_vel_rhop.bin'
            open(unit = 257,file='OUTPUT_FILES/'//outputname,status='unknown',action='write',form='unformatted',iostat=ios)
            if (ios /= 0) stop 'Error writing kernel file to disk'
          endif
          !! modified by PWY do not want diagonal Hessian
      ...
      ! attenuation kernels by PWY on 04-29-2018
      Qalpha_kl(:,:,:) = 0._CUSTOM_REAL
      Qbeta_kl(:,:,:) = 0._CUSTOM_REAL
      Qkappa_kl(:,:,:) = 0._CUSTOM_REAL
      Qmu_kl(:,:,:) = 0._CUSTOM_REAL 

      dl_lambda_kl(:,:,:) = 0._CUSTOM_REAL
      dl_mu_kl(:,:,:) = 0._CUSTOM_REAL
      dl_rho_kl(:,:,:) = 0._CUSTOM_REAL
      dip_ip_kl(:,:,:) = 0._CUSTOM_REAL
      dip_is_kl(:,:,:) = 0._CUSTOM_REAL
      dip_rho_kl(:,:,:) = 0._CUSTOM_REAL
      vipi_vp_kl(:,:,:) = 0._CUSTOM_REAL
      vipi_vs_kl(:,:,:) = 0._CUSTOM_REAL
      vipi_ip_kl(:,:,:) = 0._CUSTOM_REAL
      vipii_vp_kl(:,:,:) = 0._CUSTOM_REAL
      vipii_vs_kl(:,:,:) = 0._CUSTOM_REAL
      vipii_is_kl(:,:,:) = 0._CUSTOM_REAL
      sigma_vp_kl(:,:,:) = 0._CUSTOM_REAL
      sigma_sigma_kl(:,:,:) = 0._CUSTOM_REAL
      sigma_rho_kl(:,:,:) = 0._CUSTOM_REAL

      tti_ec_rho_kl(:,:,:) = 0._CUSTOM_REAL
      tti_ec_c11_kl(:,:,:) = 0._CUSTOM_REAL
      tti_ec_c13_kl(:,:,:) = 0._CUSTOM_REAL
      tti_ec_c15_kl(:,:,:) = 0._CUSTOM_REAL
      tti_ec_c33_kl(:,:,:) = 0._CUSTOM_REAL
      tti_ec_c35_kl(:,:,:) = 0._CUSTOM_REAL
      tti_ec_c55_kl(:,:,:) = 0._CUSTOM_REAL

      tti_ecu_rho_kl(:,:,:) = 0._CUSTOM_REAL
      tti_ecu_c11_kl(:,:,:) = 0._CUSTOM_REAL
      tti_ecu_c13_kl(:,:,:) = 0._CUSTOM_REAL
      tti_ecu_c33_kl(:,:,:) = 0._CUSTOM_REAL
      tti_ecu_c55_kl(:,:,:) = 0._CUSTOM_REAL
      tti_ecu_theta_kl(:,:,:) = 0._CUSTOM_REAL

      tti_thom_rhop_kl(:,:,:) = 0._CUSTOM_REAL
      tti_thom_alpha_kl(:,:,:) = 0._CUSTOM_REAL
      tti_thom_beta_kl(:,:,:) = 0._CUSTOM_REAL
      tti_thom_epsilon_kl(:,:,:) = 0._CUSTOM_REAL
      ...
      hti_ec_c33_kl(:,:,:) = 0._CUSTOM_REAL
      hti_ec_c55_kl(:,:,:) = 0._CUSTOM_REAL

      hti_thom_rhop_kl(:,:,:) = 0._CUSTOM_REAL
      hti_thom_alpha_kl(:,:,:) = 0._CUSTOM_REAL
      hti_thom_beta_kl(:,:,:) = 0._CUSTOM_REAL
      hti_thom_epsilon_kl(:,:,:) = 0._CUSTOM_REAL
      hti_thom_delta_kl(:,:,:) = 0._CUSTOM_REAL

      hti_thom2_rhop_kl(:,:,:) = 0._CUSTOM_REAL
      hti_thom2_alpha_kl(:,:,:) = 0._CUSTOM_REAL
      hti_thom2_beta_kl(:,:,:) = 0._CUSTOM_REAL
      hti_thom2_epsilon_kl(:,:,:) = 0._CUSTOM_REAL
      hti_thom2_delta_kl(:,:,:) = 0._CUSTOM_REAL
      hti_thom2_eta_kl(:,:,:) = 0._CUSTOM_REAL

      hti_vel_rhop_kl(:,:,:) = 0._CUSTOM_REAL
      hti_vel_alpha_kl(:,:,:) = 0._CUSTOM_REAL
      hti_vel_beta_kl(:,:,:) = 0._CUSTOM_REAL
      hti_vel_alphah_kl(:,:,:) = 0._CUSTOM_REAL
      hti_vel_alphan_kl(:,:,:) = 0._CUSTOM_REAL
      ...
      pdh_vp(:,:,:) = 0._CUSTOM_REAL
      pdh_vs(:,:,:) = 0._CUSTOM_REAL
      pdh_rhop(:,:,:) = 0._CUSTOM_REAL
      pdh_kappa(:,:,:) = 0._CUSTOM_REAL
      pdh_mu(:,:,:) = 0._CUSTOM_REAL
      pdh_rho(:,:,:) = 0._CUSTOM_REAL
      pdh_dl_lambda(:,:,:) = 0._CUSTOM_REAL
      pdh_dl_mu(:,:,:) = 0._CUSTOM_REAL
      pdh_dl_rho(:,:,:) = 0._CUSTOM_REAL
      pdh_dip_ip(:,:,:) = 0._CUSTOM_REAL
      pdh_dip_is(:,:,:) = 0._CUSTOM_REAL
      pdh_dip_rho(:,:,:) = 0._CUSTOM_REAL
      pdh_vipi_vp(:,:,:) = 0._CUSTOM_REAL
      pdh_vipi_vs(:,:,:) = 0._CUSTOM_REAL
      pdh_vipi_ip(:,:,:) = 0._CUSTOM_REAL
      pdh_vipii_vp(:,:,:) = 0._CUSTOM_REAL
      pdh_vipii_vs(:,:,:) = 0._CUSTOM_REAL
      pdh_vipii_is(:,:,:) = 0._CUSTOM_REAL
      pdh_sigma_vp(:,:,:) = 0._CUSTOM_REAL
      pdh_sigma_sigma(:,:,:) = 0._CUSTOM_REAL
      pdh_sigma_rho(:,:,:) = 0._CUSTOM_REAL

      pdh_vp_temp(:) = 0._CUSTOM_REAL
      pdh_vs_temp(:) = 0._CUSTOM_REAL
      pdh_rhop_temp(:) = 0._CUSTOM_REAL
      pdh_kappa_temp(:) = 0._CUSTOM_REAL
      pdh_mu_temp(:) = 0._CUSTOM_REAL
      pdh_rho_temp(:) = 0._CUSTOM_REAL
      pdh_dl_lambda_temp(:) = 0._CUSTOM_REAL
      pdh_dl_mu_temp(:) = 0._CUSTOM_REAL
      pdh_dl_rho_temp(:) = 0._CUSTOM_REAL
      pdh_dip_ip_temp(:) = 0._CUSTOM_REAL
      pdh_dip_is_temp(:) = 0._CUSTOM_REAL
      pdh_dip_rho_temp(:) = 0._CUSTOM_REAL
      pdh_vipi_vp_temp(:) = 0._CUSTOM_REAL
      pdh_vipi_vs_temp(:) = 0._CUSTOM_REAL
      pdh_vipi_ip_temp(:) = 0._CUSTOM_REAL
      pdh_vipii_vp_temp(:) = 0._CUSTOM_REAL
      pdh_vipii_vs_temp(:) = 0._CUSTOM_REAL
      pdh_vipii_is_temp(:) = 0._CUSTOM_REAL
      pdh_sigma_vp_temp(:) = 0._CUSTOM_REAL
      pdh_sigma_sigma_temp(:) = 0._CUSTOM_REAL
      pdh_sigma_rho_temp(:) = 0._CUSTOM_REAL

      pdh_vp_temp1(:) = 0._CUSTOM_REAL
      pdh_vs_temp1(:) = 0._CUSTOM_REAL
      pdh_rhop_temp1(:) = 0._CUSTOM_REAL
      pdh_kappa_temp1(:) = 0._CUSTOM_REAL
      pdh_mu_temp1(:) = 0._CUSTOM_REAL
      pdh_rho_temp1(:) = 0._CUSTOM_REAL
      pdh_dl_lambda_temp1(:) = 0._CUSTOM_REAL
      pdh_dl_mu_temp1(:) = 0._CUSTOM_REAL
      pdh_dl_rho_temp1(:) = 0._CUSTOM_REAL
      pdh_dip_ip_temp1(:) = 0._CUSTOM_REAL
      pdh_dip_is_temp1(:) = 0._CUSTOM_REAL
      pdh_dip_rho_temp1(:) = 0._CUSTOM_REAL
      pdh_vipi_vp_temp1(:) = 0._CUSTOM_REAL
      pdh_vipi_vs_temp1(:) = 0._CUSTOM_REAL
      pdh_vipi_ip_temp1(:) = 0._CUSTOM_REAL
      pdh_vipii_vp_temp1(:) = 0._CUSTOM_REAL
      pdh_vipii_vs_temp1(:) = 0._CUSTOM_REAL
      pdh_vipii_is_temp1(:) = 0._CUSTOM_REAL
      pdh_sigma_vp_temp1(:) = 0._CUSTOM_REAL
      pdh_sigma_sigma_temp1(:) = 0._CUSTOM_REAL
      pdh_sigma_rho_temp1(:) = 0._CUSTOM_REAL

      pdh_tti_ec_c11(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_ec_c13(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_ec_c15(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_ec_c33(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_ec_c35(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_ec_c55(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_ec_rho(:,:,:) = 0._CUSTOM_REAL

      pdh_tti_ecu_c11(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_ecu_c13(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_ecu_c33(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_ecu_c55(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_ecu_rho(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_ecu_theta(:,:,:) = 0._CUSTOM_REAL

      pdh_tti_thom_alpha(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_thom_beta(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_thom_rhop(:,:,:) = 0._CUSTOM_REAL
      pdh_tti_thom_epsilon(:,:,:) = 0._CUSTOM_REAL
      ...
      pdh_tti_ec_c33_temp(:) = 0._CUSTOM_REAL
      pdh_tti_ec_c35_temp(:) = 0._CUSTOM_REAL
      pdh_tti_ec_c55_temp(:) = 0._CUSTOM_REAL
      pdh_tti_ec_rho_temp(:) = 0._CUSTOM_REAL

      pdh_tti_ecu_c11_temp(:) = 0._CUSTOM_REAL
      pdh_tti_ecu_c13_temp(:) = 0._CUSTOM_REAL
      pdh_tti_ecu_c33_temp(:) = 0._CUSTOM_REAL
      pdh_tti_ecu_c55_temp(:) = 0._CUSTOM_REAL
      pdh_tti_ecu_rho_temp(:) = 0._CUSTOM_REAL
      pdh_tti_ecu_theta_temp(:) = 0._CUSTOM_REAL

      pdh_tti_thom_alpha_temp(:) = 0._CUSTOM_REAL
      pdh_tti_thom_beta_temp(:) = 0._CUSTOM_REAL
      pdh_tti_thom_epsilon_temp(:) = 0._CUSTOM_REAL
      ...
      pdh_tti_ec_c15_temp1(:) = 0._CUSTOM_REAL
      pdh_tti_ec_c33_temp1(:) = 0._CUSTOM_REAL
      pdh_tti_ec_c35_temp1(:) = 0._CUSTOM_REAL
      pdh_tti_ec_c55_temp1(:) = 0._CUSTOM_REAL
      pdh_tti_ec_rho_temp1(:) = 0._CUSTOM_REAL

      pdh_hti_ec_c11(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_ec_c13(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_ec_c33(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_ec_c55(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_ec_rho(:,:,:) = 0._CUSTOM_REAL

      pdh_hti_ec_c11_temp(:) = 0._CUSTOM_REAL
      pdh_hti_ec_c13_temp(:) = 0._CUSTOM_REAL
      pdh_hti_ec_c33_temp(:) = 0._CUSTOM_REAL
      pdh_hti_ec_c55_temp(:) = 0._CUSTOM_REAL
      pdh_hti_ec_rho_temp(:) = 0._CUSTOM_REAL

      pdh_hti_ec_c11_temp1(:) = 0._CUSTOM_REAL
      pdh_hti_ec_c13_temp1(:) = 0._CUSTOM_REAL
      pdh_hti_ec_c33_temp1(:) = 0._CUSTOM_REAL
      pdh_hti_ec_c55_temp1(:) = 0._CUSTOM_REAL
      pdh_hti_ec_rho_temp1(:) = 0._CUSTOM_REAL

      pdh_hti_thom_alpha(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_thom_beta(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_thom_epsilon(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_thom_delta(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_thom_rhop(:,:,:) = 0._CUSTOM_REAL

      pdh_hti_thom_alpha_temp(:) = 0._CUSTOM_REAL
      pdh_hti_thom_beta_temp(:) = 0._CUSTOM_REAL
      pdh_hti_thom_epsilon_temp(:) = 0._CUSTOM_REAL
      pdh_hti_thom_delta_temp(:) = 0._CUSTOM_REAL
      pdh_hti_thom_rhop_temp(:) = 0._CUSTOM_REAL

      pdh_hti_vel_alpha(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_vel_beta(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_vel_alphah(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_vel_alphan(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_vel_rhop(:,:,:) = 0._CUSTOM_REAL

      pdh_hti_vel_alpha_temp(:) = 0._CUSTOM_REAL
      pdh_hti_vel_beta_temp(:) = 0._CUSTOM_REAL
      pdh_hti_vel_alphah_temp(:) = 0._CUSTOM_REAL
      pdh_hti_vel_alphan_temp(:) = 0._CUSTOM_REAL
      pdh_hti_vel_rhop_temp(:) = 0._CUSTOM_REAL

      pdh_hti_thom2_alpha(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_thom2_beta(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_thom2_epsilon(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_thom2_delta(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_thom2_rhop(:,:,:) = 0._CUSTOM_REAL
      pdh_hti_thom2_eta(:,:,:) = 0._CUSTOM_REAL

      pdh_hti_thom2_alpha_temp(:) = 0._CUSTOM_REAL
      pdh_hti_thom2_beta_temp(:) = 0._CUSTOM_REAL
      pdh_hti_thom2_epsilon_temp(:) = 0._CUSTOM_REAL
      pdh_hti_thom2_delta_temp(:) = 0._CUSTOM_REAL
      pdh_hti_thom2_rhop_temp(:) = 0._CUSTOM_REAL
      pdh_hti_thom2_eta_temp(:) = 0._CUSTOM_REAL
    endif
