program misfit_adjoint_attenuation
    ! 28/04/2018
    ! by Wenyong Pan
    ! to calculate adjoint source and misfit for calculating attenuation kernels
...
end program misfit_adjoint_attenuation
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!    logical :: JOINT_MISFIT
!    real(kind=CUSTOM_REAL) :: misfit_lambda
    ...
    if(VISCOELASTIC) then
        print*
        print*,'Wenyong Pan is testing viscoelastic'
...
subroutine Yunsong_diff()
    ...
    measurement_type=trim(measurement_types(itype))
    call misfit_adj_AD(measurement_type,d,s,NSTEP,&
        VISCOELASTIC,measurement_attenuation,&
        deltat,f0,ntstart,ntend,&
        window_type,compute_adjoint, &
        misfit_value,adj)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine finalize_attenuation(directory,data_name)
    use seismo_parameters
    implicit none
    integer :: irec
    character(len=MAX_STRING_LEN) :: data_name
    ...
    if(compute_adjoint) then
        ...
        write(filename,'(a)') &
            trim(directory)//'/SEM_ATTENUATION/'//trim(filen)
...
end subroutine finalize_attenuation

subroutine finalize2(directory,data_name)
    use seismo_parameters
    implicit none
    integer :: irec
    character(len=MAX_STRING_LEN) :: data_name
...
subroutine readSU_rm(filename,seism)
    use seismo_parameters
    integer :: ier,irec
    real(kind=CUSTOM_REAL) :: seism(NSTEP,nrec_proc)
    character(len=MAX_FILENAME_LEN) :: filename
    ...
    seism = 0.0_CUSTOM_REAL
    ! open(IIN,file=trim(filename),access='direct',recl=240+4*NSTEP,iostat =
    ! ier)
    open(IIN,file=trim(filename),status='old',form='unformatted',&
        access='direct',recl=240+4*NSTEP,iostat = ier)
    ...
    close(IIN,STATUS='DELETE')

end subroutine readSU_rm
